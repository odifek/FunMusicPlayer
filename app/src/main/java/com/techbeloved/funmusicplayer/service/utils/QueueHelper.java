package com.techbeloved.funmusicplayer.service.utils;
...
import static com.techbeloved.funmusicplayer.service.utils.MediaIDHelper.MEDIA_ID_MUSICS_BY_SEARCH;
...
public class QueueHelper {
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueue(String mediaId, MusicProvider musicProvider) {
        ...
        // Contains browsable sub category
        String categoryValue = hierarchy[1];
        Log.d(TAG, "getPlayingQueue: Creating playing queue for " + categoryType);

        Iterable<MediaMetadataCompat> tracks = null;

        // More categories can be handled also
        if (categoryType.equals(MEDIA_ID_MUSICS_BY_GENRE)) {
            tracks = musicProvider.getMusicsByGenre(categoryValue);
        } else if (categoryType.equals(MEDIA_ID_MUSICS_BY_SEARCH)) {
            tracks = musicProvider.searchMusicBySongTitle(categoryValue);
        } else if (categoryType.equals(MEDIA_ID_MUSICS_ALL_SONGS)) {
            tracks = musicProvider.getAllMusics();
        }

        if (tracks ==  null) {
            Log.e(TAG, "getPlayingQueue: Unrecognised category type: " + categoryType + " for media " + mediaId);
            return null;
        }
    ...
    private static List<MediaSessionCompat.QueueItem> convertToQueue(
        ...
        for (MediaMetadataCompat track : tracks) {
            ...
            // We create a hierarchy-aware mediaID, so we know what queue is about by looking
            ...
            MediaMetadataCompat trackCopy = new MediaMetadataCompat.Builder(track)
                    ...
                    .build();
            // We don't expect queues to change after created, so we use the item index as the queueID.
            // Any other number unique in the queue would work
    ...
    public static boolean isQueueItemPlaying(Activity context, MediaSessionCompat.QueueItem queueItem) {
        ...
        if (controller != null && controller.getPlaybackState() != null) {
            ...
            String currentPlayingMediaId = controller.getMetadata()
